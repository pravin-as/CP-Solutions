...
vector<ll> helper1(ll p){
    vector<ll> temp;
    for(ll i = 1 ; i <= sqrtl(p); i++){
        if(p / i == i){
            temp.push_back(i);
        }else{
            temp.push_back(i);
...
void solve(){
    ...
    while(TC--){
            ...
            vector<ll> v = helper1(p);

            for(ll i = 0; i < sz(v); i++){

                ll temp2 = power1(2LL,p/v[i]);
                temp2 += v[i];
                ...
                if(v[i] != 2){
                    ...
                    bool flag = false;
                    if(sqrtl(n) == (ll)sqrtl(n))flag = true;
                    if(flag){
                        ans3 = 1;
                        break;
                    }
