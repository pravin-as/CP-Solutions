...
void solve(){
    ...
    while(t--){ 
        ...
        ll cnt = 0;
        bool one = (temp[0] == '1' ? true : false);

        if(one == true){
            A.push_back(power1(2LL,(ll)temp.size())-1ll);
            cnt++;
            one =  false;
        }

        for(ll i = 1 ; i < (ll)temp.size() ; i ++){
            if(temp[i]== '1' && one == true){
                cnt++;
                one = false;
                A.push_back(power1(2LL,(ll)temp.size()- i)-1LL);
            }else if(temp[i]=='0' && one == false){
                cnt++;
                one = true;
                A.push_back(power1(2LL,(ll)temp.size()-i) - 1LL);
            }
