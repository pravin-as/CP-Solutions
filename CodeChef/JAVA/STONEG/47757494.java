...
//		try {
		...
		while (t > 0) {
			...
			if (n == 3) {
				...
				if (A[1] <= Math.max(A[0], A[2])) {
					pt.println(Math.max(A[0], A[2]) - A[1] + 1);
				} else {
					pt.println(0);
				}
				...
				long increase[] = A.clone();
				long decrease[] = A.clone();
				long pos[] = new long[n];
				long neg[] = new long[n];
