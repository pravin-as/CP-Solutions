...
//		try {
		...
		while (t > 0) {
			...
			int n = fr.nextInt();
			long min = Long.MAX_VALUE, min_odd = Long.MAX_VALUE, min_even = Long.MAX_VALUE, min_freq = 0;
			long A[] = new long[n];
			for (int i = 0; i < n; i++) {
				A[i] = fr.nextInt();
			...
			for (int i = 0; i < n; i++) {
				if (A[i] == min) {
					...
					if (A[i] % 2 == 1) {
						...
						min_even = Math.min(min_even, A[i]);
					}
				}
			}
			pt.println(Math.min(min_odd / 2, min_even / 2 - 1) < min ? n : n - min_freq);
