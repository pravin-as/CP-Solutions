...
//			}
			...
			double ans1 = Math.ceil(((double) b - (double) x) / (double) 2)
					+ Math.ceil(((double) x - (double) a) / (double) 2);

			double ans2 = Integer.MIN_VALUE;

			if (x - 1 >= Math.min(a, b)) {
				y--;
				ans2 = Math.ceil(((double) b - (double) y) / (double) 2)
						+ Math.ceil(((double) y - (double) a) / (double) 2);
			}

			double ans3 = Integer.MIN_VALUE;

			if (x + 1 <= Math.max(a, b)) {
				z++;
				ans3 = Math.ceil(((double) b - (double) z) / 2) + Math.ceil(((double) z - (double) a) / 2);

			}

			pt.println((int) (Math.max(ans1, Math.max(ans2, ans3))));
