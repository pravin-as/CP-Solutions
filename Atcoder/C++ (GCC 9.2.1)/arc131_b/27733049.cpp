...
void dfs(ll x, ll y){
    ...
    if(check(x,y)){
        if(mat[x][y] == '.'){
            for(ll i = 1; i <= 5; i++){
                bool flag = true;
                for(ll j = 0; j < 4; j++){
                    if(check(x+dx[j],y+dy[j])&&mat[x+dx[j]][y+dy[j]] == '.'){

                    }else
                    if(check(x+dx[j],y+dy[j])&&mat[x+dx[j]][y+dy[j]] == (char)('0'+i)){
                        flag = false;
                    }
                }
    ...
    for(ll i = 0; i < 4; i++){
        if(check(x+dx[i],y+dy[i])&&mat[x+dx[i]][y+dy[i]]=='.'&&!vis[x+dx[i]][y+dy[i]])
            {dfs(x+dx[i],y+dy[i]);}
    }
...
void solve(){
    ...
    while(t--){
        ...
        vis.clear();vis.resize(n+1,vector<ll>(m+1));
        adj.clear();adj.resize(n+1);
        mat.clear();mat.resize(n+1);
        
        for(ll i = 1; i <= n; i++){mat[i].push_back('0');}
