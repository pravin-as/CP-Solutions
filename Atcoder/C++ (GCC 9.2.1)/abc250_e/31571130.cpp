...
void solve(){
    ...
    while(TC--){
            ...
            ll n, q; cin >> n;
            vector<ll> A(n), B(n), pref_A(n), pref_B(n);
            ...
            for(ll i = 0; i < n; i++){
                if(mp.find(A[i]) == mp.end()){
                    mp[A[i]] = 1023LL * A[i] + 511LL * (A[i] / 31LL) * (A[i] + 69LL); 
                }
                if(mp.find(B[i]) == mp.end()){
                    mp[B[i]] = 1023LL * B[i] + 511LL * (B[i] / 31LL) * (B[i] + 69LL); 
                }
                pref_A[i] = (i == 0 ? 0LL : pref_A[i - 1]) + (s_A.find(mp[A[i]]) == s_A.end() ? mp[A[i]] : 0LL);
                pref_B[i] = (i == 0 ? 0LL : pref_B[i - 1]) + (s_B.find(mp[B[i]]) == s_B.end() ? mp[B[i]] : 0LL);
                s_A.insert(mp[A[i]]);
                s_B.insert(mp[B[i]]);
            }
