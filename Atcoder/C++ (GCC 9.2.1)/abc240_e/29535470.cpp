...
void dfs(ll node){
    ...
    if(sz(adj[node]) == 1 && node != 1){
        ans[node].ff = num;
        ans[node].ss = num;
        debug(ans[node])debug(node)
        num++;
    }


    for(auto child: adj[node]){
        if(sz(adj[node]) == 1 && node != 1)continue;
        debug(child)debug(node)
        ans[node].ff = min(ans[node].ff,ans[child].ff); 
        ans[node].ss = max(ans[node].ss,ans[child].ss);                     
    }
