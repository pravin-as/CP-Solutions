...
void bfs(ll node){
    ...
    queue<pair<ll,ll>> q; //pair can also be stored according to problem.
    q.push({node,0LL});
    ll ans = -1;
    ...
    while(!q.empty()){
        ...
        if(temp[sz(temp)-1]!='0'){
            temp = temp.substr(sz(temp)-1) + temp.substr(0,sz(temp)-1);
            node = stoll(temp);
            if(!vis[node]){
                vis[node] = 1;
                q.push({node,val+1});
...
void solve(){
    ...
    while(TC--){
       ...
       cin >> a >> n;
       adj.assign(n+1,vector<ll>()); dp.assign(n+1,0LL); vis.assign(1e6+5,0LL);
