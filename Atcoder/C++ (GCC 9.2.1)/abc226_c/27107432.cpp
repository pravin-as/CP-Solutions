...
string ds = "RLDU";
vector<vector<ll>> tot(5e5+7);
vector<ll> vis(5e5+7),tim(5e5+7);
ll sm = 0LL;
...
void solve(){
    ...
    while(t--){
        ...
        tot.resize(n+1);vis.resize(n+1);tim.resize(n+1);
        ...
        for(ll i = 0; i < n; i++){
            ...
            tim[i+1] = tm1;
            ll k;cin>>k;
            tot[i+1].push_back(i+1);
            for(ll j = 0; j < k; j++){
                ll temp1;cin>>temp1;
                tot[i+1].push_back(temp1);
            }
        }
        ...
        for(ll i = 1 ; i < sz(tot[n]); i++){
            if(vis[tot[n][i]] == 1)continue;
            debug(tot[n][i])
            debug(i)
            dfs(tot[n][i]);
