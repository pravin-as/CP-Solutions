...
void solve(){
    ...
    for (int i = 1; i <= N; i++) {
        ...
        else if (i % 2 == 1 && Primes[i / 2] == 0)prime.push_back(i);
    }
    sort(all(prime));
    ...
    for(ll j = 0; j < sz(prime); j++){
        for(ll i = 0; i < j; i++){
            if((ll)prime[j] * (ll)prime[j] > ll(1e18) / ((ll)prime[j] * (ll)prime[i])){break;}
            v.push_back((ll)prime[i] * (ll)prime[j] * (ll)prime[j] * (ll)prime[j]);
        }
    }

   // debug(cnt)debug(sz(prime))debug(sz(v))
