...
void solve(){
   ...
   // cin >> TC;
    ...
    while(TC--){
            ...
            ll i = 1, j = 1, cnt1 = 1, cnt2 = 1;

            while(true){

                bool flag = false;

                for(; i <= min(cnt1 * a + (cnt1 - 1) * c,x); i++){
                    dis1 += b;
                    flag = true;
                }
                for(; i <= min(cnt1 * a + cnt1 * c, x); i++){
                    dis1 += 0;
                }
                cnt1++;


                for(; j <= min(cnt2 * d + (cnt2 - 1) * f,x); j++){
                    dis2 += e;
                    flag = true;
                }
                for(; j <= min(cnt2 * d + cnt2 * f, x); j++){
                    dis2 += 0;
                }
                cnt2++;
            ...
            cout << (dis1 == dis2 ? "Draw" : dis1 > dis2 ? "Takahashi" : "Aoki") << "\n";
