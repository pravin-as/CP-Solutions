...
void solve(){
    ...
    while(t--){
        ...
        for(ll i = 1; i < n; i++){
            if(A[i]==0){
                for(ll j = 1; j < m + 1; j++){
                    dp[i][j] = (j-1>=1?dp[i-1][j-1]:0LL) + dp[i-1][j] + (j+1<=m?dp[i-1][j+1]:0LL);
                    dp[i][j] %= MOD; 
                }
            }else{
                dp[i][A[i]] += dp[i-1][A[i]] + (A[i]-1>=1?dp[i-1][A[i]-1]:0LL) + (A[i]+1<=m?dp[i-1][A[i]+1]:0LL);
                dp[i][A[i]] %= MOD;
            }
        }
