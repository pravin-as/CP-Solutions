...
void solve(){
    ...
    while(TC--){
              ...
              dp[0][0] = 0;
               for(ll j = 1; j <= m; j++){
                    (dp[0][j] += dp[0][j - 1]) %= MOD1;
                }
              for (ll i = 1; i < n; i++) {
                  for (ll j = 1; j <= m; j++) {
                    if(p == 0){
                        (dp[i][j] += dp[i-1][j - p]) %= MOD1;
                        (dp[i][j] += mod_sub(dp[i-1][m], dp[i - 1][min(j + p,m)], MOD1)) %= MOD1;
                        continue;
                    }
                    if(j - p >= 1){
                        (dp[i][j] += dp[i-1][j - p]) %= MOD1;
                    }
                    if(j + p <= m)(dp[i][j] += mod_sub(dp[i-1][m], dp[i - 1][min(j + p - 1,m)], MOD1)) %= MOD1;
