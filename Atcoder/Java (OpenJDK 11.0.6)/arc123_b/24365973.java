...
//		try {
		...
		while (t > 0) {
			...
			int count = 0, indexb = -1, indexc = -1;

			for (int i = 0; i < n; i++) {
				indexb = upper_bound(B, indexb + 1, n - 1, A[i]);
				if (indexb > n - 1 || indexb == -1) {
					break;
				}
				indexc = upper_bound(C, indexc + 1, n - 1, B[indexb]);

				if (indexb == -1 || indexc == -1) {
					break;
				} else {
					count++;
//					pt.println(indexb + " " + indexc);
