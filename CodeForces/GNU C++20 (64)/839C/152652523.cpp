...
lld dfs(ll node, ll val){
    ...
    for(auto child: adj[node]){
        if(vis[child]==0){
            temp += dfs(child, node) + 1; //Change val according to problem.
        }
    }

    return temp ? (lld)(temp) / (lld)((sz(adj[node]) - (val != -1))) : 0LL;
...
void solve(){
    ...
    while(TC--){
            ...
            cout<<fixed<<setprecision(18)<<dfs(1,-1)<<"\n";
