...
void solve(){
    ...
    ll mx = 32768;
    vector<ll> dp(mx + 1, MOD), dp2(mx + 1);
    dp[mx] = 0; dp2[mx] = 0;
    dp[0] = 0;

    for(ll i = mx; i >= 0; i--){
        if(i * 2LL <= mx && i + 1LL <= mx){
            dp[i] = min(1LL + min(dp[i+1], dp[i * 2LL]), dp[i]);
        }else if(i + 1 <= mx){
            dp[i] = min(1LL + dp[i + 1], dp[i]);
        }
    }
    ...
    while(i1 >= 0){
        while(j1 > i1){
            dp2[i1] = min(dp2[i1], j1 - i1 + dp2[j1]);
            j1--;
        }
        i1--;
    }
