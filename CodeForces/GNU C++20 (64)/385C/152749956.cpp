...
void solve(){
    ...
    while(TC--){
            ...
            ll n; cin >> n;
            ll number_of = 10000007;
            vector<ll> A(n), vis(number_of);
            for(ll i = 0; i < n; i++){
                cin >> A[i];
                vis[A[i]]++;
            }
            ll q; cin >> q;
            SieveOfEratosthenes(number_of);
            // debug(primes)

            vector<ll> freq(number_of + 1), pref(number_of + 1);
            for(auto i: primes){
                ll num = i;
                while(num < number_of){
                    if(vis[num] != 0)freq[i]+=vis[num];
                    num += i;
                }
            }
