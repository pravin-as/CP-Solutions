...
void solve(){
    ...
    while(TC--){
            ...
            ll ans = 0;
            for(ll i = 1; i < n; i++){
                ll fir = dis[i], sec = dis[i+1];
                s.erase(s.find(fir));s.erase(s.find(sec));
                s.insert(fir + sec + 1);
                auto it = s.end(); it--;
                ll large = *it;large--;
                ll put = max(d - A[n] - 1, large / 2);
                s.insert(put);
                ans = max(ans, *s.begin());
                s.erase(s.find(put)); s.erase(s.find(fir + sec + 1));
                s.insert(fir); s.insert(sec);
            }

            s.erase(s.find(dis[n]));
            s.insert(d - A[n-1] - 1);
            ans = max(ans , *s.begin());
            cout << ans << "\n";
