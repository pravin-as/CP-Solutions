    ...
    while(TC--){
            ...
            while(l <= r){
                ...
                for(ll i = 0; i < m; i++){
                    ...
                    ll left_ind = (i == 0 ? -1:  zero[i - 1]), 
                        p = min(i + mid - 1, m - 1),
                            right_ind = (p == m - 1 ? -1 : zero[p + 1]);

                    ll left_ones = (i == 0 ? 0 : left_ind + 1 - i);
                    ll right_ones = (p == m - 1 ? 0 : n - right_ind - (m - (p + 1)));

                    if(left_ones + right_ones <= mid){
                        flag = true;
                        break;
                    }
                }
