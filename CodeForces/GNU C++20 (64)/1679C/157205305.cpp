    ...
    while(TC--){
            ...
            while(q--){
                ...
                if(type == 1){
                    ...
                    ll x, y; cin >> x >> y; x--; y--; A[x]++; B[y]++;

                    if(A[x] == 1)X.update(0,x,x,1LL,0,n - 1);
                    if(B[y] == 1)Y.update(0,y,y,1LL,0,n - 1);

                }else if(type == 2){

                    ll x, y; cin >> x >> y; x--; y--; A[x]--; B[y]--;

                    if(A[x] == 0)X.update(0,x,x,-1LL,0,n - 1);
                    if(B[y] == 0)Y.update(0,y,y,-1LL,0,n - 1);

                }else{

                    ll x1, x2, y1, y2; cin >> x1 >> y1 >> x2 >> y2; x1--; y1--; x2--; y2--;

                    ll x_s = X.query(0, min(x1, x2), max(x1, x2), 0, n - 1).sm;
                    ll y_s = Y.query(0, min(y1, y2), max(y1, y2), 0, n - 1).sm;

                    debug(x_s)debug(y_s)

                    if(y_s >= abs(y2 - y1) + 1 || x_s >= abs(x2 - x1) + 1){
                        cout << "Yes" << "\n";
                    }else{
                        cout << "No" << "\n";
                    }
