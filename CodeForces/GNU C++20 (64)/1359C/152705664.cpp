...
void solve(){
    ...
    while(TC--){
            ...
            ll h, c, t, ans = 1e9, diff = 1e9; cin >> h >> c >> t;

            ll x1 = t / (h + c), x2 = t / (h + c) + 1;
            ll temp1 = (h + c) * x1, temp2 = (h + c) * x2;

            ll x3 = (t - c) / (h + c), x4 = (t - c) / (h + c) + 1;
            ll temp3 = (h + c) * x3 + c, temp4 = (h + c) * x4 + c;
            ...
            if(abs(t - temp3) <= diff){
                diff = abs(t - temp3);
                ans = min(ans, 2 * x3 + 3LL);
            }
            if(abs(t - temp4) <= diff){
                diff = abs(t - temp4);
                ans = min(ans, 2 * x4 + 3LL);
            }
