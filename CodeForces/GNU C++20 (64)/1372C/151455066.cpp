...
void solve(){
    ...
    while(TC--){
            ...
            bool pref = true, suff = true, mid = true;

            ll ind = 0;
            while(ind < n && A[ind] == ind + 1){
                ind++;
            }
            for(ll i = ind; i < n; i++){
                if(A[i] == i + 1)pref = false;
            }
            ll ind2 = n - 1;
            while(ind2 >= 0 && A[ind2] == ind2 + 1){
                ind2--;
            }
            for(ll i = ind2; i >= 0; i--){
                if(A[i] == i + 1)suff = false;
            }
            for(ll i = ind; i <= ind2; i++){
                if(A[i] == i + 1)mid = false;
            }

            cout << (is_sorted(all(A)) ? 0 : (pref || suff || mid) ? 1 : 2)  << "\n";
