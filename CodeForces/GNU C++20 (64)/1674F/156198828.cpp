...
void solve(){
    ...
    while(TC--){
            ...
            while(q--){
                ...
                ll col_req = cnt / n, last_col_req = cnt % n, temp_cnt = 0;            

                for(ll i = 1; i <= col_req; i++){
                    temp_cnt += n - star[i];
                }
                for(ll j = 1; j <= last_col_req; j++){
                    if(mat[j][col_req + 1] == '.')temp_cnt++;
                }
                // debug(cnt)
                // debug(col_req)debug(last_col_req)
                // debug(star)
