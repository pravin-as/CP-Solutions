...
void dfs(ll node){
    ...
    dp[node].ff += (col[node] == 1 ? 1 : 0);
    dp[node].ss += (col[node] == 0 ? 1 : 0);

    // debug(dp[node])

    for(auto child: adj[node]){
        dp[node].ff += dp[child].ff;   
        dp[node].ss += dp[child].ss;                         
    }
...
void solve(){
    ...
    while(TC--){
            ...
            for(ll i = 1; i < sz(dp); i++){
                if(dp[i].ff == dp[i].ss)cnt++;
            }
