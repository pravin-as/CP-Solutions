...
void solve(){
    ...
    while(TC--){
            ...
            ll n, k, cnt = 0, p = 0; cin >> n >> k;
            char pp = 'a';
            string str; cin >> str;

            if(k >= 25){
                for(ll i = 0; i < n; i++)str[i] = 'a';
                cout << str << "\n";
                continue;
            }
            ...
            for(ll i = 0; i < n; i++){
                ...
                if((ll)(str[i] - 'a') <= k){
                    ...
                    char nw = (char)(str[i] - k);
                    debug(i)debug(nw)
                    for(ll j = i; j < n; j++){
                        if((ll)(str[j] - nw) <= k && str[j] != 'a' && (ll)(str[j] - nw) >= 0){
                            nw = str[j];
                        }
                    }
                    k -= (pp - 'a');
                    char o = str[i] - (k);
                    for(char c = str[i]; c > o; c--){
                        for(ll jj = 0; jj < n; jj++){
                            if(str[jj] == c){
                                str[jj] = (char)(c - 1);
                            }
