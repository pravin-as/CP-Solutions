...
void solve(){
    ...
    while(TC--){
            ...
            bool f = true;
            for(auto i: str){
                if(special.find(i) != special.end())f=false;
            }
            if(f){
                cout << 0 << "\n";
                continue;
            ...
            for(ll i = 0; i <= n; i++){
                ...
                if(special.find(str[i]) != special.end()){

                    ll cnt = 0;i++;
                    bool flag = false;
                    if(i == 1)flag=true;
                    while(i <= n + 1 && special.find(str[i]) == special.end()){
                        i++;
                        cnt++;
                    }
                    if(i == n + 2)flag=true;
                    i--;
                    debug(cnt)
                    if(i + 1 <= n && special.find(str[i + 1]) != special.end())mx = max(mx, cnt+!flag);
                }
            }
