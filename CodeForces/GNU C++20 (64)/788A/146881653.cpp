...
void solve(){
    ...
    while(TC--){
            ...
            vector<vector<ll>> dp(2LL,vector<ll>(n,0LL));

            for(ll i = 0; i < n - 1; i++) {
                if(i % 2 == 1) {
                    dp[0][i] =  abs(A[i] - A[i+1]);
                    dp[1][i] = -abs(dp[0][i]);
                } else {
                    dp[0][i] = -abs(A[i] - A[i+1]);
                    dp[1][i] =  abs(dp[0][i]);
            ...
            debug(dp)debug(dp[0])debug(dp[1])
            cout << max(maxSubArraySum(dp[0],sz(dp[0])),maxSubArraySum(dp[1],sz(dp[1]))) << "\n";
