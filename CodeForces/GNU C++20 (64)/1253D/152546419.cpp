...
pair<ll,ll> dfs(ll node, ll mx, ll mn){

    if(vis[node]==1)return {mx, mn};
    vis[node] = 1;
    ...
    for(auto child: adj[node]){
        if(vis[child]==0){
            pair<ll,ll> pr2 = dfs(child, mx, mn); //Change val according to problem.
            pr = {max(pr.ff,pr2.ff), min(pr.ss, pr2.ss)};
        }
    }
...
void solve(){
    ...
    while(TC--){
            ...
            for(ll i = 1; i <= n; i++){
                if(vis[i])continue;
                pair<ll,ll> p = dfs(i, 0LL, 1e9+7);
                v.push_back({p.ss, p.ff});
            }
