...
void solve(){
    ...
    while(TC--){
            ...
            vector<vector<ll>> dp(n + 1, vector<ll>(h + 1, -1LL));
            ...
            for(ll i = 2; i <= n; i++){
                ...
                for(ll j = 0; j < h; j++){
                    ...
                    ll a = (j + A[i]) % h, b = (j + A[i] - 1LL) % h;

                    if(a >= l && a <= r){
                        dp[i][a] = max(dp[i][a], 1LL + dp[i - 1][j]);
                    }else{
                        dp[i][a] = max(dp[i][a], dp[i - 1][j]);
                    }

                    if(b >= l && b <= r){
                        dp[i][b] = max(dp[i][b], 1LL + dp[i - 1][j]);
                    }else{
                        dp[i][b] = max(dp[i][b], dp[i - 1][j]);
                    }
