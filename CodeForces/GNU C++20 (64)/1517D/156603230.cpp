...
void solve(){
    ...
    while(TC--){
            ...
            ll n, m, K; cin >> n >> m >> K;
            ll dp[n + 3][m + 3][K + 3];
            vector<vector<ll>> A(n+1, vector<ll>(m + 1, 0LL)), B(n+1, vector<ll>(m + 1, 0LL));
            ...
            for(ll k = 2; k <= K; k+=2){
                for(ll i = 1; i <= n; i++){
                    for(ll j = 1; j <= m; j++){
                        dp[i][j][k] = min({(i > 1 ? 2LL * B[i - 1][j] + dp[i - 1][j][k - 2] : INF)
                            , (j > 1 ? 2LL * A[i][j - 1] + dp[i][j - 1][k - 2] : INF)
                               , (i < n ? 2LL * B[i][j] + dp[i + 1][j][k - 2] : INF)
                                   , (j < m ? 2LL * A[i][j] + dp[i][j + 1][k - 2] : INF)});
                    }
                }
            }
