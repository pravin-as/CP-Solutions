...
void solve(){
    ...
    while(TC--){
            ...
            num_at_n = min(n,num_at_n);
            num_at_n = max(1LL,num_at_n);
            ans[n] = num_at_n;

            ll cnt = 2;

            for(ll i = num_at_n; i <= n + 1; i++){

                A[n] = cnt;
                if(cnt > n)continue;
                ll temp = query(A,n);

                ans[temp] = num_at_n + cnt - 1;
                cnt++;
