...
void bfs(vector<ll> nodes){
    ...
    while(!q.empty()){
        ...
        q.pop();
        vis2[node]=1;
        if(vis[node] == 0)ans.insert(node);


        

        if(vis.find(node + 1) == vis.end() && vis2.find(node + 1) == vis2.end()){
            q.push(node + 1);
        }
        if(vis.find(node - 1) == vis.end() && vis2.find(node - 1) == vis2.end()){
            q.push(node - 1);
