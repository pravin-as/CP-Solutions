...
void solve(){
    ...
    while(TC--){
            ...
            for(ll i = 1; i < n; i++){
                dp0[i] = min(dp0[i-1] + abs(A[i] - A[i-1]) + abs(B[i] - B[i-1]),
                            dp1[i-1] + abs(A[i] - B[i-1]) + abs(B[i] - A[i-1]));
                dp1[i] = min(dp0[i-1] + abs(B[i] - A[i-1]) + abs(A[i] - B[i-1]),
                            dp0[i-1] + abs(B[i] - B[i-1]) + abs(A[i] - A[i-1]));
