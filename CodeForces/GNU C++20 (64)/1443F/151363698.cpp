...
void solve(){
    ...
    while(TC--){
            ...
            for(ll i = 0; i < k; i++){
                ll num = B[i];
                pre.erase(pre.find(num));
                auto it = s.find({ind[num],num});

                if(it == s.begin()){
                    auto it2 = it; it2++;
                    if(pre.find((it2 -> ss)) == pre.end()){
                        ans = mod_mul(ans,1LL,MOD1);
                        s.erase(it2);
                    }else{
                        ans = mod_mul(ans,0LL,MOD1);
                    }
                    continue;
                }
                auto last = s.end();
                ...
                if(pre.find((it_prev -> ss)) == pre.end() && pre.find((it_next -> ss)) == pre.end()){
                    ans = mod_mul(ans, 2LL, MOD1);
                    s.erase(it_prev);
                }else if(pre.find((it_prev -> ss)) != pre.end() && pre.find((it_next -> ss)) != pre.end()){
                    ans = mod_mul(ans, 0LL, MOD1);
                }else{
                    if(pre.find((it_prev -> ss)) != pre.end()){
                        ans = mod_mul(ans, 1LL, MOD1);
                        s.erase(it_next);
                    }else{
                        ans = mod_mul(ans, 1LL, MOD1);
