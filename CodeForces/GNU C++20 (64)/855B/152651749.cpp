...
void solve(){
    ...
    while(TC--){
            ...
            ll n, p, q, r, ans = -8e18; cin >> n >> p >> q >> r;
            vector<ll> A(n), left_min(n), left_max(n), right_min(n), right_max(n);
            ...
            // debug(left_max)debug(right_max)debug(left_min)debug(right_min)


            for(ll i = 0; i < n; i++){
                ll temp = q * A[i];
                temp += max(p * left_max[i], p * left_min[i]);
                temp += max(r * right_max[i], r * right_min[i]);
                ans = max(temp, ans);
            }
