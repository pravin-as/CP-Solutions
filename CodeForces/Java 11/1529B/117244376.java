	...
	public static void main(String[] args) {
		...
		try {
			...
			while (t > 0) {
				...
				long min = (long) 1e10 + 7, minPositive = pos.isEmpty() ? (long) 1e10 + 7 : pos.get(0);

				for (int i = 0; i < neg.size() - 1; i++) {
					long pr = Math.abs(neg.get(i + 1) - neg.get(i));
					min = Math.min(min, pr);

				}

				pt.println(n == 1 ? 1 : min >= minPositive ? ans + neg.size() + 1 : ans + neg.size());
