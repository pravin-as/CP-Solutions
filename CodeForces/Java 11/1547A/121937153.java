...
public class Ginny_Weasley {
	...
	static boolean yo(String a, String b) {
...
//		try {
		...
		while (t > 0) {
			...
			int xa = fr.nextInt(), ya = fr.nextInt(), xb = fr.nextInt(), yb = fr.nextInt(), xf = fr.nextInt(),
					yf = fr.nextInt();

			if (xa == xb && xa == xf && yf > Math.min(ya, yb) && yf < Math.max(ya, yb)) {
				pt.println(Math.abs(ya - yb) + 2);

			} else if (ya == yb && ya == yf && xf > Math.min(xa, xb) && xf < Math.max(xa, xb)) {
				pt.println(Math.abs(xa - xb) + 2);
			} else {
				pt.println(Math.abs(xa - xb) + Math.abs(ya - yb));
			}
