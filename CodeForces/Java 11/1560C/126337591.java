...
//		try {
		...
		while (t > 0) {
			...
			long small = (long) Math.sqrt(k) * (long) Math.sqrt(k);
			long big = ((long) Math.sqrt(k) + 1) * ((long) Math.sqrt(k) + 1);
			long mid = small + 1 + (big - small + 1) / 2;

			if (Math.sqrt(k) == (long) Math.sqrt(k)) {
				pt.println((long) Math.sqrt(k) + " " + 1);
			} else

			if (k >= mid) {
				pt.println((long) Math.sqrt(big) + " " + (big - k + 1));
			} else {
				pt.println((k - small) + " " + (long) Math.sqrt(big));
			}
