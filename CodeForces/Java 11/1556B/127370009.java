...
//		try {
		...
		while (t > 0) {
			...
			if (Math.abs(odd - even) <= 1) {
				...
				if (odd > even) {
					long odd_ind = 0, even_ind = 1, ans = 0;

					for (int i = 0; i < n; i++) {
						if (A[i] % 2 == 0) {
							ans += Math.abs(even_ind - i);
							even_ind += 2;
						} else {
							ans += Math.abs(odd_ind - i);
							odd_ind += 2;
						}
					}
					pt.println(ans / 2);
					...
					long odd_ind = 1, even_ind = 0, ans = 0;
					...
					long odd_ind1 = 1, even_ind1 = 0, ans1 = 0, ans2 = 0, odd_ind2 = 0, even_ind2 = 1;

					for (int i = 0; i < n; i++) {
						if (A[i] % 2 == 0) {
							ans1 += Math.abs(even_ind1 - i);
							even_ind1 += 2;
						} else {
							ans1 += Math.abs(odd_ind1 - i);
							odd_ind1 += 2;
						}
					}
					...
					for (int i = 0; i < n; i++) {
						if (A[i] % 2 == 0) {
							ans2 += Math.abs(even_ind2 - i);
							even_ind2 += 2;
						} else {
							ans2 += Math.abs(odd_ind2 - i);
							odd_ind2 += 2;
						}
					}

					pt.println(Math.min(ans2, ans1) / 2);
