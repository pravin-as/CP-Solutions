...
void solve(){
    ...
    while(TC--){
        ...
        ll n, required = 0, ans = 0; cin >> n;
        vector<ll> k(n), h(n), suff(n+1,INF);
        ...
        required = k[n-1] - h[n-1] + 1;


        for(ll i = n - 1; i >= 0; i--){
            suff[i] = min(suff[i+1],k[i]-h[i]+1);
        }
        debug(suff)
        ...
        for(ll i = n - 1; i >= 0; i--){
            ...
            while(j >= 0 && curr <= k[j]){
                curr = min(curr,suff[j]);
                j--;
            }
            ll temp = ((k[i] - (suff[j+1]) + 1));
            ans += (temp * (temp + 1)) / 2;
            i = j + 1;
