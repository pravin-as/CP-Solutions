...
void dfs(ll i, ll j){
    ...
    for(ll ind = 0; ind < 4; ind++){
        if(check(i+dx[ind],j+dy[ind]) && adj2[i+dx[ind]][j+dy[ind]] == '.'){
            cnt++;
        }
    }
    // debug(vis[i][j])

    if(cnt <= 1 && check(i,j) && vis[i][j] == 0){
        vis[i][j] = 1;
        adj2[i][j] = '+';
    ...
    for(ll ind = 0; ind < 4; ind++){
        if(check(i+dx[ind],j+dy[ind]) && adj2[i+dx[ind]][j+dy[ind]] != '#')dfs(i+dx[ind],j+dy[ind]);
    }
}
}
...
void solve(){
    ...
    while(t--){
            ...
            for(ll i = 0; i < 4; i++){
                if(check(labx+dx[i],laby+dy[i])&&adj2[labx+dx[i]][laby+dy[i]] != '#')dfs(labx+dx[i],laby+dy[i]);
            }
