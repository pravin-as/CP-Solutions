...
vector<ll> vis(5e5+7,0LL), roots(5e5+7,0LL);
...
void solve(){
    ...
    while(t--){
        ...
        adj.clear();adj.resize(n+1);vis.clear();vis.resize(n+1);roots.clear();roots.resize(0);


        for(ll i = 1; i <= n; i++){
            ll par; cin >> par;
            if(par!=-1){adj[par].push_back(i);}else{roots.push_back(i);}
