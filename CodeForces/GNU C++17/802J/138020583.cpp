...
vector<ll> vis(5e5+7),dp(5e5+7);map<pair<ll,ll>,ll> val;
ll n, ans = 0;
...
void dfs(ll child, ll parent){
    ...
    for(auto i: adj[child]){
        if(!vis[i]){dfs(i,child);
        dp[child] = max(dp[i] + val[{child,i}],dp[child]);
        }
    }
...
void solve(){
    ...
    while(t--){
        ...
        cin >> n;
        adj.clear();adj.resize(n);vis.clear();vis.resize(n);val.clear();dp.clear();dp.resize(n);
