...
void solve(){
    ...
    while(TC--){ll n, k, mn = INF; cin >> n >> k;vector<ll> A(N), B(n), C(N);for(ll i = 0; i < n; i++){cin >> B[i];A[B[i]]++;}sort(B.begin(),B.end());for(ll i = 0; i < n; i++){ll temp = B[i], cnt = 0;while(temp > 0){temp /= 2LL;cnt++;A[temp]++;if(A[temp] <= k){C[temp] += cnt;}}}for(ll i = 0; i < N; i++){if(A[i] >= k){mn = min(mn,C[i]);}}cout << mn << "\n";}}
