...
ll minDifference(vector<ll> arr, ll n)
{
    ll sum = 0;
    for (ll i = 0; i < n; i++)
        sum += arr[i];
...
void solve(){
    ...
    while(TC--){
            ...
            ll n, sm = 0, ans = 0; cin >> n;
            vector<ll> A(n), B(n), C(n);
            vector<vector<bool>> dp(n+1,vector<bool>(100*(n+1LL)));
