...
vector<ll> vis(5e5+7), mark(5e5 + 7);
ll cnt = 0, ans = 1, n, m, cnt2 = 0;


void dfs2(ll child, ll parent){



    vis[child] = 1;

    if(sz(adj[child]) == 1){cnt2++;
        // debug(cnt2)debug(child)debug(parent)
    }
...
void dfs(ll child, ll parent, ll common){

    // debug(child)debug(common)


    vis[child] = 1;
    if(common>m)return;

    if(sz(adj[child]) == 1){cnt++;
        debug(child)debug(common)debug(cnt)
    }

    for(auto i: adj[child]){
        if(!vis[i]){
            if(mark[i]){dfs(i,child,common+1);}
            else{dfs(i,child,0);}
        }
    }
...
void solve(){
    ...
    while(t--){
        ...
        cin >> n >> m;
        adj.clear();adj.resize(n+1);vis.clear();vis.resize(n+1);mark.clear();mark.resize(n+1);

        for(ll i = 1; i <= n; i++)cin >> mark[i];
        for(ll i = 1; i < n; i++){
            ll a, b; cin >> a >> b;
            adj[a].push_back(b);
            adj[b].push_back(a);
