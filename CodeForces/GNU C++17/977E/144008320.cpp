...
ll n, m; bool flag = true, visit = false; ll cnt = 0;

void dfs(ll node){
    if(vis[node]==1)return;
    vis[node] = 1;
    if(deg[node]!=2)flag=false;visit = true;
...
void solve(){
    ...
    while(TC--){
        ...
        cin >> n >> m; 
        adj.assign(n+1,vector<ll>()); vis.assign(n+1,0LL); deg.assign(n+1,0LL);
        

        for(ll i = 1; i <= m; i++){
            ll a, b; cin >> a >> b;debug(a)
            adj[a].push_back(b);
            adj[b].push_back(a);
            deg[a]++;deg[b]++;
        }
