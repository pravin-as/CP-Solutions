...
string ds = "RLDU";
vector<vector<ll>> adj(5e0+7);
vector<ll> vis(5e0+7,0LL);
...
void dfs(ll node){
    ...
    cnt++;
        ...
        for(auto child : adj[node]){
            if(vis[child]==0 && mp[{node,child}] == 0){
                ans = max(ans,cnt+1);
                dfs(child);
...
void solve(){
    ...
    while(t--){
        ...
        vector<pair<ll,ll>> v; vector<ll> ap;
        for(ll i = 0; i < n - 1; i++){
            ll x, y, val; cin >> x >> y >> val;
            adj[x].push_back(y);
            adj[y].push_back(x);
            mp[{x,y}] = val;mp[{y,x}] = val;
        }
        debug(adj)
        ...
        cout << (power(n,k) % MOD - sub % MOD + MOD ) % MOD << "\n";
