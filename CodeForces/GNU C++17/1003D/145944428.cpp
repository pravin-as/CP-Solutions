...
void solve(){
    ...
    while(TC--){
            ...
            while(q--){
                ...
                ll k; cin >> k;
                string str = decToBinary(k);
                ll num = 1, cnt = 0;
                debug(str)
                freq = tm;
                bool flag = true;

                for(ll i = 30; i >= 0 && k > 0; i--){
                    ll temp = (min(k/power1(2LL,i),freq2[i]));
                    cnt += temp;
                    k -= (power1(2LL,i)) * temp;
                }
                ...
                for(ll i = sz(str) - 1; i >= 0; i--){
                    if(str[i] == '1'){
                        ll temp = num, p = num;debug(num)
                        while(num > 0 && temp > 0){
                            cnt += min(freq[temp],num / temp);
                            num -= min(freq[temp],num / temp) * temp;
                            freq[temp] -= min(freq[temp],num / temp);
                            temp /= 2LL;
                            // debug(num)debug(freq)debug(temp)
                        }
                        if(num != 0){
                            flag = false;
                            break;
