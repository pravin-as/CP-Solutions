...
void solve(){
    ...
    while(TC--){
        ...
        ll n; cin >> n; set<string> s, p;bool flag = false;for(ll i = 0; i < n; i++){string str, temp; cin >> str; temp = str; reverse(temp.begin(),temp.end());s.insert(str);if(s.find(temp)!=s.end()){flag = true;}if(sz(str) == 3){p.insert(str.substr(0,2));if(s.find(temp.substr(0,2)) != s.end()){flag = true;}}if(sz(str) == 2){if(p.find(temp) != p.end()){flag = true;}}}cout << (flag ? "YES" : "NO") << "\n";
