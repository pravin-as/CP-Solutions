...
void solve(){
    ...
    while(TC--){
        ...
        ll n, length, k; cin >> n >> length >> k;

        vector<ll> dist(n+5), tim(n+5);
        ...
        vector<vector<ll>> dp(n+5,vector<ll>(k+5,INF));  // dp[i][j] means minimum time taken
                          // to reach i'th barrier and removing
                          // j barriers 
        ...
        for(ll i = 1; i <= n + 1; i++){
            ...
            for(ll j = 0; j <= k; j++){
                ...
                for(ll p = i-1; p >= 0; p--){
                  ...
                  if(j-(i-p-1)>=0)dp[i][j] = min(dp[p][j-(i-p-1)] + (dist[i]-dist[p]) * tim[p],dp[i][j]);
                  debug(i)debug(j)debug(p)debug(dp[i][j])debug(j-(i-p-1))
                }
