...
void build(ll ind,ll start,ll end){
    ...
    seg[ind] = __gcd(seg[2*ind+1],seg[2*ind+2]); // operation may change
}
ll query(ll ind,ll left, ll right, ll start,ll end){
    if(left<=start && right>=end){
        ...
        ll mid = (start+end) >> 1;
        return __gcd(query(2*ind+1,left,right,start,mid) , query(2*ind+2,left,right,mid+1,end));// operation may change
    ...
    while(TC--){
        ...
        for(ll i = 0; i < n; i++){
            ...
            while(l <= r){
                ...
                ll mid = l + (r - l) / 2;
                ll temp = seg.query(0,mid,i,0,n-1), len = i - mid + 1;

                bool check = (seg.query(0,mid,i,0,n-1) > (len) ? true : false); 
