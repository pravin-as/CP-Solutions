...
void solve(){
    ...
    while(t--){
        ...
        for(ll i = 0; i < n; i++){
            ...
            A.push_back({temp2-temp1,{temp1,temp2}});
        }
        ...
        for(ll i = 0; i < sz(A); i++){
            ll temp; ll temp2 = A[i].second.second; ll p = 0;
            auto it = s.begin();
            if(s.find(temp2)==s.end()){
                it = s.lower_bound(temp2);it--;p = *it;s.erase(it);
            }else{
                p = temp2;s.erase(temp2);
            }
            cout << A[i].second.first << " " << temp2 << " " << p << "\n";
