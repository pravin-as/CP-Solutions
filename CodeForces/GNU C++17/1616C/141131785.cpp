...
// vector<ll>  dp(5e5+7,0LL);
vector<vector<ll>> vis;
vector<vector<char>> mat;
vector<double> arr;


int cal(int n, double d)
{
    ...
    while(t--){
         ...
         for(int i = 0; i < n; i++){
            for(int j = i + 1; j < n; j++){
                int cnt = 0;
                for(int k = 0; k < n; k++){
                    if((arr[j]-arr[i])*(k-i)==(arr[k]-arr[i])*(j-i))continue;
                    cnt++;
                }
                ans=min(ans,cnt);
         ...
         // for(double d = (double)-200; d <= (double)200; d+=1){
         //    // debug(d)
         //    for(double j = d; j <= d + 1; j+=0.5)
         //    ans = min(ans,cal(n,j));
         // }

         ans=min(ans,cal(n,(double)0));
