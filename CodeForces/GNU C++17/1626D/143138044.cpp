...
void solve(){
    ...
    while(TC--){
        ...
        ll n; cin >> n;
        vector<ll> A(n), B, pow(21); map<ll,ll> mp;
        ...
        for(ll i = 0; i <= 20; i++){
            ...
            for(ll j = 0; j <= 20; j++){
                ...
                for(ll k = 0; k <= 20; k++){
                    ...
                    ll light = pow[i], mid = pow[j], high = pow[k];

                    ll fir = upper_bound(B.begin(),B.end(),light) - B.begin();
                    ll fir_val = (fir == 0 ? 0 : B[fir-1]);

                    // if(B[fir]<light)continue;

                    ll sec = upper_bound(B.begin(),B.end(),fir_val+mid) - B.begin();
                    ll sec_val = (sec == 0 ? 0 : B[sec-1]);
                    sec_val -= fir_val;

                    // if(B[fir]<mid)continue;

                    ll thi =  B.back() - fir_val - sec_val;

                    if(thi > high)continue;

                    ans = min(ans,light+mid+high-fir_val-sec_val-thi);               
        ...
        // Sometimes calculating factors is better while sometimes calculating multiples is better, depends on constraints.
