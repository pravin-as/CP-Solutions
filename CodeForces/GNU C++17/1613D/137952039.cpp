...
void solve(){
    ...
    while(t--){
            ...
            ll n, ans = 0; cin >> n;
            vector<ll> A(n+2), dp1(n+2), dp2(n+2); for(ll i = 1; i <= n; i++)cin >> A[i];
            dp1[0] = 1;

            for(ll i = 1; i <= n; i++){
                if (A[i] > 0)
            {
                dp2[A[i] - 1] = (dp1[A[i] - 1] + 2 * dp2[A[i] - 1]) % MOD1;
            }
            dp1[A[i] + 1] = (dp1[A[i]] + 2 * dp1[A[i] + 1]) % MOD1;
            dp2[A[i] + 1] = (2 * dp2[A[i] + 1]) % MOD1;
            }
