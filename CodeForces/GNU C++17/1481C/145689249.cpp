...
//    cout<<"called"<<endl;
    ...
    bhag(n, 0)cin>>vecx[i];
    bhag(n, 0)cin>>vecy[i];
    map<ll, ll>vec_colour;
    bhag(n, 0)
    {
        vec_colour[vecy[i]] =  i+1 ;
    }
    bhag(k, 0)cin>>vecp[i];
    map<ll,queue<ll>>needs;
    vector<ll>coloured(n);
    ll requirement = 0;
    for(auto i = 0 ; i < n  ; i++)
    {
        if(vecx[i]!=vecy[i])
    ...
    {
        if(!needs[vecp[i]].empty())
        {
 
            ll index = needs[vecp[i]].front();
            ans[i] =  index;
            needs[vecp[i]].pop();
            coloured[index-1]--;
    ...
    missed.push_back(k-1);
    bool truth =  false;
    if(!needs[vecp[k-1]].empty())
    {
        //   cout<<"for"<<endl;
        ll index =  needs[vecp[k-1]].front() ;
        if(coloured[index-1] > 0)coloured[index-1]--;
        for(auto i : missed)
        {
            ans[i]  = needs[vecp[k-1]].front();
        }
 
    }
    else if(vec_colour[vecp[k-1]])
    {
        //cout<<"checked here "<<endl;
        for(auto i : missed)
        {
            ans[i] =  vec_colour[vecp[k-1]];
        }
